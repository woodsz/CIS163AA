/* 
 * ComputeTax.java
 * Programming Excerise 3.13
 *   Prompts the user to enter filing status
 *   and taxable income and computes the tax
 */
...
public class ComputeTax {
	...
	public static void main(String[] args) {
		...
		else if(status == 1){ // Compute taxes married filing jointly
			if(income <= 16700)
				tax = income * 0.10;
		...
		else if(status == 2){ // Compute taxes filing separately
			if(income <= 8350)
				tax = income * 0.10;
			else if(income <= 33950)
				tax = 8350 * 0.10 + (income - 8350) * 0.15;
		...
		else if(status == 3){ // Compute taxes for head of household
			if(income <= 11950)
				tax = income * 0.10;
